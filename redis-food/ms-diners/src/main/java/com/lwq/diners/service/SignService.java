...
import com.lwq.commons.constant.PointTypesConstant;
...
public class SignService {
    ...
    public int doSign(String accessToken, String dateStr) {
        ...
        // 调用远程积分服务，添加签到积分并返回--这里可以用异步实现，使用队列去做，没必要这里等着
    ...
    private int getContinuousSignCount(Integer dinerId, Date date) {
        ...
        for (int i = dayOfMonth; i > 0; i--) {// i 表示位移操作次数
            // 右移再左移，如果等于自己说明最低位是 0，表示未签到，不信的可以试试
            if (v >> 1 << 1 == v) {
                // 低位 0 且非当天说明连续签到中断了
            ...
            // 右移一位并重新赋值，相当于把最低位丢弃一位，这一天已经算完了，下一次循环算下一天
            v >>= 1;
        }
        return signCount;
    }
    ...
    private String buildSignKey(Integer dinerId, Date date) {
        return String.format("user:sign:%d:%s", dinerId,
                DateUtil.format(date, "yyyyMM"));//格式到月，就是一个月一个月的存
    }
